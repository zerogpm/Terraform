...
module "alb" {
  ...
  https_listener_rules = [
    ...
    # Rule-3: When Query-String, ?google=gg redirect to https://google.com
    {
      https_listener_index = 0
      priority             = 3
      actions = [{
    ...
    # Rule-4: When Host Header = app1.devopeasyway.com, redirect to https://www.binaryunicornsoftware.com/
    {
      https_listener_index = 0
      priority             = 4
      actions = [{
        type        = "redirect"
        status_code = "HTTP_302"
        host        = "www.binaryunicornsoftware.com"
        path        = "/"
        query       = ""
        protocol    = "HTTPS"
      }]
